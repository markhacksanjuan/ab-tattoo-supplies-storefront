...
export async function getNavigationTree() {
    ...
    return rootCategories.map(root => {
        ...
        // Get direct children of root (may be leaf categories OR group categories)
        const directChildren = (root.category_children || [])
        ...
        // Determine if this is a 3-level type (root → groups → leaves)
        // by checking for category_groups metadata.
        let categoryGroups = null
        let children
        let rawGroups = root.metadata?.category_groups
        if (rawGroups) {
            if (typeof rawGroups === 'string') {
                try { rawGroups = JSON.parse(rawGroups) } catch {
                    console.warn(
                        `[getNavigationTree] category_groups for "${root.name}" is not valid JSON. ` +
                        `Make sure all keys are quoted: {"groupLabel": "...", "handles": [...]}`
                    )
                    rawGroups = null
                }
            }
        ...
        if (rawGroups && Array.isArray(rawGroups)) {
            // 3-level type (e.g. Material → Groups → Leaves)
            // Direct children are GROUP categories; their children are the
            // actual navigable leaf categories.
            const allLeaves = directChildren.flatMap(group =>
                (group.category_children || [])
                    .filter(c => c.is_active !== false)
                    .sort((a, b) => (a.rank ?? 0) - (b.rank ?? 0))
                    .map(c => ({
                        handle: c.handle,
            ...
            categoryGroups = rawGroups.map(group => ({
                ...
                categories: (group.handles || [])
                    .map(h => allLeaves.find(c => c.handle === h))
                    .filter(Boolean),
            }))

            // Flat list of all leaves (used by ProductFilters, etc.)
            children = allLeaves
        } else {
            // 2-level type (e.g. Agujas, Tintas → Leaves)
