...
export async function initializePaymentSession(cartId, providerId = 'pp_stripe_stripe') {
    try {
        // Retrieve cart to get payment collection ID
        ...
        const response = await fetch(`${MEDUSA_BACKEND_URL}/store/payment-collections/${cart.payment_collection.id}/payment-sessions`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-publishable-api-key': PUBLISHABLE_KEY,
