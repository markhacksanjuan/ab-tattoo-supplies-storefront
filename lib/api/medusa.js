...
export async function initializePaymentSession(cartId, providerId = 'pp_stripe_stripe', paymentCollectionId = null) {
    try {
        // Use provided paymentCollectionId or retrieve cart to get it
        let pcId = paymentCollectionId
        if (!pcId) {
            const cart = await getCart(cartId)
            pcId = cart?.payment_collection?.id
        }
        if (!pcId) {
