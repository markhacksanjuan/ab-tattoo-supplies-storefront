...
export async function getProducts(params = {}) {
    try {
        ...
        // filter (it silently ignores it). We capture it here and apply
        // client-side filtering after the API response.
        ...
        // When client-side type filtering is active the server count is
        // unreliable (it includes all types). We fall back to the filtered
        // length in that case, but preserve the real server count otherwise
        // so pagination works correctly.
        const totalCount = clientTypeFilter ? filteredProducts.length : (count ?? filteredProducts.length)
...
export async function initializePaymentSession(cartId, providerId = 'pp_stripe_stripe') {
    try {
        ...
        if (!pcId) {
            console.error('Could not create or find payment collection for cart')
            return null
        }
        ...
        if (!response.ok) {
            const errorText = await response.text()
            console.error('Failed to initialize payment session:', errorText)
            return null
        }
 ...
 */
export function findBestVariant(product, desiredOptions = {}) {
    ...
    // Fallback: find variant that matches the most recently changed option
    // (the last entry in desiredOptions)
    const entries = Object.entries(desiredOptions)
    const lastEntry = entries[entries.length - 1]
    if (lastEntry) {
        const [lastOptId, lastVal] = lastEntry
        const partial = product.variants.find(variant =>
            variant.options?.some(vo => vo.option_id === lastOptId && vo.value === lastVal)
        )
